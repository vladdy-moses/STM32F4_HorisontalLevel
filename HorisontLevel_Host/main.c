...
int main(void) {
	...
	/* Инициплизация акселерометра */
	accelerometer_Init();
	accelerometer_SendData(0x20, 0x67);

	char str[256];
	Accelerometer_Result result;

	while (1) {
		if (0 == downTicker) {
			result = accelerometer_Work();
			sprintf(str, "%3.7f,\t%ld,\t%d,\t%d,\t%d\r\n", result.angle,
					result.deviation, result.x, result.y, result.z);
			VCP_send_str((uint8_t *) str);
			downTicker = 5;
		}
	}
